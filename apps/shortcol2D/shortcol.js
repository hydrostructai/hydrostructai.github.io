...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderPreviewOnly() {
        ...
        ShortColOut.drawCrossSection('svg-preview-container', state.colType, state.geometry, bars, state.reinforcement.d_bar);
        
        // Update Info text
        const totalAs = bars.length * state.reinforcement.As_bar;
        const ag = state.colType === 'rect' ? (state.geometry.B * state.geometry.H) : (Math.PI * state.geometry.D**2 / 4);
        const ratio = (totalAs / ag) * 100;
        dom.spanReinfoInfo.innerHTML = `As tổng = <b>${totalAs.toFixed(0)}</b> mm² (${ratio.toFixed(2)}%)`;
    }
    ...
    // Inputs Change
    const inputs = [
        dom.inpB, dom.inpH, dom.inpD, dom.inpCover, 
        dom.inpRb, dom.inpRs,
        dom.inpNb, dom.inpdBar
    ];
    ...
    dom.btnCalc.addEventListener('click', () => {
        ...
        setTimeout(() => {
            try {
                ...
                if (loads.length === 0) throw new Error("Vui lòng nhập ít nhất 1 tổ hợp tải trọng.");

                // 3. Tính toán Curve
                const curvePoints = ShortColCal.calculateInteractionCurve(
                    state.colType, 
                    state.geometry.B, state.geometry.H, state.geometry.D,
                    state.material.fck, state.material.fyk, 
                    bars
                );
                ...
                ShortColOut.renderChart('interactionChart', curvePoints, loads, safetyFactors);
