/**
 * ShortCol Calculation Module - 3 STANDARDS SUPPORT
...
const ShortColCal = {
  /**
   * LẤY THAM SỐ TIÊU CHUẨN - Get Standard Parameters
   * @param {string} standard - "TCVN", "EC2", or "ACI"
   * @param {number} fck - Cường độ bê tông đặc trưng (MPa)
   * @param {number} fyk - Cường độ thép đặc trưng (MPa)
   * @returns {Object} Tham số chuẩn hóa
   */
  getStandardParams: function (standard, fck, fyk) {
    const Es = 200000; // Modul đàn hồi thép (MPa)

    if (standard === "TCVN") {
      // TCVN 5574:2018 - Vietnam Standard
      return {
        standard: "TCVN 5574:2018",
        Rb: fck, // Cường độ thiết kế bê tông = fck
        Rs: fyk, // Cường độ thiết kế thép = fyk
        e_cu: 0.0035, // Biến dạng cực hạn bê tông
        beta: 0.8, // Hệ số khối ứng suất α = 0.8
        Es: Es,
        phi_design: 1.0, // Hệ số an toàn thiết kế (TCVN: không dùng φ riêng)
        description:
          "Chuẩn Việt Nam: Rb = fck, Rs = fyk, a = 0.8c, εcu = 0.0035",
      };
    } else if (standard === "EC2") {
      // EC2:2004/2015 - Eurocode 2
      const gamma_c = 1.5; // Hệ số bảo toàn bê tông
      const gamma_s = 1.15; // Hệ số bảo toàn thép

      return {
        standard: "EC2:2004/2015",
        Rb: 0.85 * (fck / gamma_c), // fcd = 0.85 × (fck / 1.5) ≈ 0.567 × fck
        Rs: fyk / gamma_s, // fyd = fyk / 1.15 ≈ 0.87 × fyk
        e_cu: 0.0035, // Biến dạng cực hạn (C50, có thể thay đổi cho C90)
        beta: 0.8, // Hệ số khối ứng suất λ = 0.8
        Es: Es,
        phi_design: 1.0, // Hệ số an toàn (tích hợp vào fcd, fyd)
        description:
          "Chuẩn EU: fcd = 0.567×fck, fyd = 0.87×fyk, a = 0.8×x, εcu = 0.0035",
      };
    } else if (standard === "ACI") {
      // ACI 318-19 - American Concrete Institute
      // ACI sử dụng Nominal capacity (chưa nhân φ)
      const beta1 = this.calculateBeta1(fck); // Hệ số phụ thuộc f'c

      return {
        standard: "ACI 318-19",
        Rb: 0.85 * fck, // σ_c = 0.85 × f'c
        Rs: fyk, // Không chia γ (dùng giá trị đặc trưng)
        e_cu: 0.003, // ACI dùng 0.003 (nhỏ hơn TCVN/EC2)
        beta: beta1, // Hệ số khối ứng suất phụ thuộc f'c
        Es: Es,
        phi_design: 0.75, // Hệ số hiệu chỉnh φ = 0.75 cho uốn + nén
        description:
          "Chuẩn Mỹ: σc = 0.85×f'c, a = β1×c, εcu = 0.003, β1 phụ f'c",
      };
    } else {
      // Mặc định TCVN
      return this.getStandardParams("TCVN", fck, fyk);
    }
  },

  /**
   * TÍNH HỆ SỐ β1 THEO ACI (phụ thuộc f'c)
   * ACI 318-19: Section 22.2.2.4.3
   * @param {number} fck - Cường độ bê tông (MPa)
   * @returns {number} Hệ số β1
   */
  calculateBeta1: function (fck) {
    if (fck <= 28) {
      return 0.85;
    } else if (fck <= 55) {
  ...
  /**
   * TÍNH TOÁN BIỂU ĐỒ TƯƠNG TÁC (P-M) - CORE LOGIC
   * Hỗ trợ 3 tiêu chuẩn: TCVN, EC2, ACI
   * @param {string} standard - "TCVN", "EC2", "ACI"
   * @param {string} type - "rect" hoặc "circ"
   * @param {number} B - Chiều rộng (mm) hoặc đường kính
   * @param {number} H - Chiều cao (mm)
   * @param {number} D - Đường kính (cột tròn)
   * @param {number} fck - Cường độ bê tông đặc trưng (MPa)
   * @param {number} fyk - Cường độ thép đặc trưng (MPa)
   * @param {Array} bars - Mảng vị trí cốt thép
   * @returns {Array<{x: number, y: number}>} Mảng điểm (M kNm, P kN)
   */
  calculateInteractionCurve: function (
    standard,
    type,
    B,
    H,
    ...
    // Lấy tham số tiêu chuẩn
    const params = this.getStandardParams(standard, fck, fyk);
    const Rb = params.Rb; // Cường độ thiết kế bê tông
    const Rs = params.Rs; // Cường độ thiết kế thép
    const e_cu = params.e_cu; // Biến dạng cực hạn
    const beta = params.beta; // Hệ số khối ứng suất
    const Es = params.Es; // Modul thép
    ...
    for (let xi of xi_steps) {
      ...
      if (c > 0) {
        // Chỉ tính bê tông khi có vùng nén (c > 0)
        if (type === "rect") {
          // --- CỘT CHỮ NHẬT ---
          // Diện tích nén = B * a_eff
          // a_eff = β × c (β = 0.8 cho TCVN/EC2, phụ thuộc f'c cho ACI)
          let a_eff = beta * c;
          if (a_eff > H) a_eff = H;
