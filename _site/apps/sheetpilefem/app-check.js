/*
 * app-check.js (cho Sheet Pile FEM - REFACTORED ARCHITECTURE)
 *
 * Responsibilities:
 ...
 * 4. Enforce FREE mode restrictions (max 2 soil layers)
 */

const LICENSE_CONFIG = {
    APP_NAME: 'sheetpile',
    STORAGE_KEY: 'sheetpileLicensed',
    FREE_LIMIT: 2,  // Max soil layers in FREE mode
...
async function handleLicenseCheck() {
    ...
    try {
        ...
        if (mode === 'PRO') {
            ...
            localStorage.setItem(LICENSE_CONFIG.STORAGE_KEY, 'false');
            updateLicenseStatusUI('error', 'Key không hợp lệ. Phiên bản FREE giới hạn 2 lớp đất.');
        }
    }
}
