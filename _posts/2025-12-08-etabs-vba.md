...
Trong Excel, mở trình soạn thảo VBA (Alt + F11). Bạn không cần thêm Reference ETABSv1.dll. Tuy nhiên, để thao tác với file text dễ dàng, nên bật thư viện Microsoft Scripting Runtime (Tools > References > Microsoft Scripting Runtime).
...
Do While Not txtStream.AtEndOfStream
    ...
    ' Gọi hàm xử lý dữ liệu tại đây (Parsing)
Loop
 ...
 * Nếu là chuỗi (String): Loại bỏ dấu "" bao quanh.
...
>  * LinkedIn Discussion: Vivek Patel's Post - Automation Expert for Civil Engineers
> 

### Các điểm đã chỉnh sửa để khớp với bài ShortCol:
1.  **Front Matter (Phần đầu file):**
    * Thêm `layout: single` (để đảm bảo hiển thị đúng layout bài viết chi tiết).
    * Thêm `author_profile: true` (để hiện sidebar thông tin tác giả bên trái).
    * Chuẩn hóa `header` với `overlay_image` giúp bài viết đồng bộ giao diện với các bài khác.
2.  **Định dạng Code Block:** Sử dụng ` ```vba ` và ` ```text ` để highlight màu code chính xác.
3.  **Trích dẫn (Blockquote):** Đưa phần tài liệu tham khảo vào trong thẻ `>` để tạo điểm nhấn cuối bài.
4.  **Hình ảnh:** Sử dụng đường dẫn tương đối `/assets/...` chuẩn của Jekyll.
